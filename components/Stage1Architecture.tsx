...
import { callGemini, callImageGen, safeJsonParse } from '../services/gemini';
...
export const Stage1Architecture: React.FC<Props> = ({ project, updateProject, setLoading, setActiveStep, setEditWorldModal }) => {
  ...
  const handleBlender = async () => {
    ...
    const text = await runAI("正在搅拌灵感...", prompt, "Creative Director", false);
    if (text) { updateProject({ ...project, idea: text }); setShowBlender(false); }
  };

  const generateArchitecture = async () => {
    if (!project.idea) return alert("请先输入核心灵感");
    const prompt = `基于创意：${project.idea}。构建小说架构，返回 JSON: { "title": "书名", "worldBible": { "time": "", "location": "", "rules": "" }, "mainPlot": "主线梗概", "characterList": [ { "name": "主角名", "role": "主角", "traits": "", "bio": "" } ], "timeline": "" }`;
    const text = await runAI("构建架构中...", prompt, "Architect", true);
    if (!text) return;
  ...
  const generateDetailedStructure = async () => {
    ...
    const text = await runAI("正在深化剧情结构...", prompt, "Structure Master", false);
    if (text) {
        updateProject({
            ...project,
            architecture: {
  ...
  const autoDeduceCharacters = async () => {
    ...
    返回 JSON 数组: [ { "name": "", "role": "", "traits": "", "bio": "" } ]`;
    
    const text = await runAI("正在推导核心人物...", prompt, "Character Expert", true);
    if (!text) return;
  ...
  const generateSingleCharacter = async (idx: number) => {
    ...
    返回 JSON: { "name": "姓名", "role": "定位", "traits": "性格", "bio": "小传" }`;
    const text = await runAI(`正在设计角色...`, prompt, "Character Designer", true);
    if (!text) return;
    const data = safeJsonParse(text);
    if (data) {
            ...
            <div className="bg-white p-5 rounded-xl border shadow-sm h-[600px] flex flex-col">
              <div className="flex justify-between mb-3 border-b pb-2">
                ...
                <div className="flex gap-1">
                  ...
                  <Button size="sm" variant="secondary" onClick={() => updateProject({...project, characterList:[...project.characterList, {id:Date.now(), name:"新角色", role:"配角", traits:"", bio:"", imageUrl: ""}]})}><Plus size={14}/></Button>
                </div>
              </div>
